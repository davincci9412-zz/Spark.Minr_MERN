	...
	Wallet.find().lean().sort({ create : "asc"}).then((wallets)=>{
		...
		if (wallet_number >0){
			...
			if (type == 0){
						...
						promiseForeach.each(saved_roi,
							...
							(err, newList) => {
								if (err) { console.log(err);	
									console.log("ROI BNB fail"+ new Date() + "________")
								}
								if (newList.length == saved_roi.length) {
									Roi.insertMany(buy_values).then(function(){
										...
										Wallet.updateOne({wallet_address:search_wallet_address}, wallet).then((result)=>{
											console.log("ROI BNB success________"+ new Date() + "________")
										})
									}).catch(function(error){
										console.log("ROI BNB fail________"+ new Date() + "________")
									});	
								} else {
									console.log("ROI BNB fail"+ new Date() + "________")
								}
						...
						Roi.find({wallet_address: search_wallet_address}).lean().sort({timeStamp:"asc"}).then((rois)=>{
							...
							if (missed_roi.length>0){
								...
								promiseForeach.each(missed_roi,
									...
									(err, newList) => {
										if (err) { console.log(err);	
											console.log("Missed ROI BNB fail"+ new Date() + "________")
										}
										if (newList.length == missed_roi.length) {
											Roi.insertMany(buy_values).then(function(){
												wallet = {rois:before_number+missed_roi.length, status:before_status}
												Wallet.updateOne({wallet_address:search_wallet_address}, wallet).then((result)=>{
													console.log("Missed ROI BNB success________"+ new Date() + "________")
												})
											}).catch(function(error){
												console.log("Missed ROI BNB fail"+ new Date() + "________")
											});	
								...
								wallet = {status:1}
								Wallet.updateOne({wallet_address:search_wallet_address}, wallet).then((result)=>{
									console.log("ROI BNB complete________"+ new Date() + "________")
								})
							}
						})
					}
				...
				TokenTransactions.find({wallet_address: search_wallet_address}).lean().sort({timeStamp:"asc"}).then((transactions)=>{
					...
					if (saved_roi.length > 0){
						...
						promiseForeach.each(saved_roi,
							...
							(err, newList) => {
								if (err) { console.log(err);	
									console.log("ROI Token fail"+ new Date() + "________")
								}
								if (newList.length == saved_roi.length) {
									Roi.insertMany(buy_values).then(function(){
										wallet = {tokenRois:before_number+k, roiTokenTimeStamp:last_timeStamp, status:before_status}
										Wallet.updateOne({wallet_address:search_wallet_address}, wallet).then((result)=>{
											console.log("ROI Token success________"+ new Date() + "________")
										})
									}).catch(function(error){
										console.log("ROI Token fail"+ new Date() + "________")
									});	
						...
						Roi.find({wallet_address: search_wallet_address}).lean().sort({timeStamp:"asc"}).then((rois)=>{
							...
							if (missed_roi.length>0){
								...
								promiseForeach.each(missed_roi,
									...
									(err, newList) => {
										if (err) { console.log(err);	
											console.log("Missed ROI Token fail"+ new Date() + "________")
										}
										if (newList.length == missed_roi.length) {
											Roi.insertMany(buy_values).then(function(){
												wallet = {tokenRois:before_number+missed_roi.length, status:before_status}
												Wallet.updateOne({wallet_address:search_wallet_address}, wallet).then((result)=>{
													console.log("Missed ROI Token success________"+ new Date() + "________")
												})
											}).catch(function(error){
												console.log("Missed ROI Token fail"+ new Date() + "________")
											});	
								...
								wallet = {status:2}
								Wallet.updateOne({wallet_address:search_wallet_address}, wallet).then((result)=>{
									console.log("ROI Token complete________"+ new Date() + "________")
								})
							}
						})
					}
				...
				console.log("Don't have the ROI to add________"+ new Date() + "________")
			}
		} else {
			console.log("Don't have any wallet________"+ new Date() + "________")
		}
	})
					...
					if (api_array.length == newList.length){
						Tokengecko.collection.drop().then(function(){
							Tokengecko.insertMany(prices).then(function(){
								console.log("Token success_______________________"+ new Date() + "__________________________");  		//success
							}).catch(function(error){
								console.log("Token fail _________________________"+ new Date() + "_____" + error);       //error
							});
						});						
					}					
				}
	...
	Wallet.find().lean().then(results => {
        if (results.length >0){
			promiseForeach.each(results,
				result => { 
					return updateBNBBalance(result.wallet_address);
				},
				(arrResult, result) => {					
				},
				(err, newList) => {
					if (results.length == newList.length){
						console.log("BNB Balance success_________"+ new Date() + "_________");  		//success
					} else {
						console.log("BNB Balance fail_________"+ new Date() + "_________")       // fail
					}				
				}
			)
		}
  ...
  balance: function(){
	...
	Wallet.find().lean().then(results => {
        if (results.length >0){
			promiseForeach.each(results,
				result => { 
					return updateBalance(result.wallet_address);
				},
				(arrResult, result) => {					
				},
				(err, newList) => {
					if (results.length == newList.length){
						console.log("Balance success___________"+ new Date() + "___________");  		//success
					} else {
						console.log("Balance fail___________"+ new Date() + "___________")       // fail
					}				
				}
			)
		}
  ...
  transactions: function(){
	...
	Wallet.find().lean().then(results => {
        if (results.length >0){
			promiseForeach.each(results,
				result => { 
					return updateTransactions(result.wallet_address, result.timeStamp);
				},
				(arrResult, result) => {					
				},
				(err, newList) => {
					if (results.length == newList.length){
						console.log("Transactions success___________"+ new Date() + "___________");  		//success
					} else {
						console.log("Transactions fail___________"+ new Date() + "___________")       // fail
					}				
				}
			)
		}
  ...
  tokenTransactions: function(){
	...
	Wallet.find().lean().then(results => {
        if (results.length >0){
			promiseForeach.each(results,
				result => { 
					return updateTokenTransactions(result.wallet_address, result.tokenTimeStamp);
				},
				(arrResult, result) => {					
				},
				(err, newList) => {
					if (results.length == newList.length){
						console.log("Token transations success___________"+ new Date() + "___________");  		//success
					} else {
						console.log("Token transactions fail___________"+ new Date() + "___________")       // fail
					}				
				}
			)
		}
...
async function updateTransactions(wallet_address, timeStamp) {
	return await new Promise( function (resolve, reject) {
		//var url  ='https://api.covalenthq.com/v1/56/address/'+wallet_address+'/balances_v2/?nft=true&no-nft-fetch=true&key=ckey_daca4efda85b4837961727b8cbb'
